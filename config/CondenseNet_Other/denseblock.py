...
class DenseBlock(nn.Sequential):
    def __init__(self, num_layers, in_channels, growth, args):
        super(DenseBlock, self).__init__()

        for layer_id in range(num_layers):
            layer = DenseLayer(in_channels=in_channels + (layer_id * growth), 
            growth=growth, args=args)
            self.add_module('dense_layer_%d' % (layer_id + 1), layer)

class DenseLayer(nn.Module):
    def __init__(self, in_channels, growth, args):
        super(DenseLayer, self).__init__()
        self.args = args
        self.bottleneck = args.bottleneck
        self.group_1x1 = args.group_1x1
        self.group_3x3 = args.group_3x3
        self.condense_factor = args.condense_factor
        self.dropout_rate = args.dropout_rate

        # 1x1 conv in_channels --> bottleneck*growth
        self.conv_1 = LearnedGroupConv(in_channels=in_channels, out_channels=self.bottleneck * growth, kernel_size=1,
                                       groups=self.group_1x1, condense_factor=self.condense_factor, dropout_rate=self.dropout_rate)

        self.batch_norm = nn.BatchNorm2d(self.bottleneck * growth)
        self.relu = nn.ReLU(inplace=True)

        # 3x3 conv bottleneck*growth --> growth
        self.conv_2 = nn.Conv2d(in_channels=self.bottleneck * growth, out_channels=growth, kernel_size=3, padding=1, stride=1, groups=self.group_3x3, bias=False)
