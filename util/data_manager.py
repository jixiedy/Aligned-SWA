...
class Zeroeasy(object):
    ...
    def _process_dir(self, dir_path, relabel=False):
        ...
        for img_path in img_paths:
            pid, camid = map(int, pattern.search(img_path).groups())
            if pid == -1: continue  # junk images are just ignored
            assert 0 <= pid <= 12826  # pid == 0 means background
            assert 1 <= camid <= 20
            camid -= 1 # index starts from 0
            if relabel: pid = pid2label[pid]
            dataset.append((img_path, pid, camid))
...
class Vehicleid(object):
    """
    Reference:
    Liu et al. Deep Relative Distance Learning: Tell the Difference Between Similar Vehicles, IEEE 2016
    ...
    def __init__(self, root='data', **kwargs):
        self.dataset_dir = osp.join(root, self.dataset_dir)
        # self.train_dir = osp.join(self.dataset_dir, 'train')
        # self.train_dir = osp.join(self.dataset_dir, 'train_8')
        # self.train_dir = osp.join(self.dataset_dir, 'train_part_id>=10')
        # self.train_dir = osp.join(self.dataset_dir, 'train_part_id>=5')
        # self.train_dir = osp.join(self.dataset_dir, 'train_later')

        self.train_dir = osp.join(self.dataset_dir, 'train_3')
        self.query_dir = osp.join(self.dataset_dir, 'mini_query_800')
        self.gallery_dir = osp.join(self.dataset_dir, 'mini_test_800')
        
        # self.train_dir = osp.join(self.dataset_dir, 'train_part_id>=10')       
        # self.query_dir = osp.join(self.dataset_dir, 'small_query_1600')
        # self.gallery_dir = osp.join(self.dataset_dir, 'small_test_1600')
        # self.query_dir = osp.join(self.dataset_dir, 'medium_query_2400')
        # self.gallery_dir = osp.join(self.dataset_dir, 'medium_test_2400')
        # self.query_dir = osp.join(self.dataset_dir, 'small_query_5')
        # self.gallery_dir = osp.join(self.dataset_dir, 'small_test_5')
    ...
    def _process_dir(self, dir_path, relabel=False):
        ...
        for img_path in img_paths:
            pid, _ = map(int, pattern.search(img_path).groups())
            # print('pattern_search', pattern.search(img_path))
            # print('pattern_search_groups', pattern.search(img_path).groups())
            # print('pid, _', map(int, pattern.search(img_path).groups()))
            if pid == -1: continue  # junk images are just ignored
            pid_container.add(pid)
        pid2label = {pid:label for label, pid in enumerate(pid_container)}
        ...
        dataset = []
        # print('pid', len(list(map(int, pattern.search(img_path).groups()))))
        for img_path in img_paths:
            # pid, camid = map(int, pattern.search(img_path).groups())
            pid, _ = map(int, pattern.search(img_path).groups())

            if pid == -1: continue  # junk images are just ignored
            assert 0 <= pid <= 27954  # pid == 0 means background
            # assert 0 <= camid <= 100
            # camid -= 1 # index starts from 0
            if relabel: pid = pid2label[pid]
            # dataset.append((img_path, pid, camid))
            dataset.append((img_path, pid, _))
...
class VRID(object):
    """
    VRID data set is provided by OpenITS
    URL: http://www.openits.cn/openData4/748.jhtml

    Dataset statistics:
    # identities: 1000 
    # images: 7000 (train) + 900 (query) + 2100 (gallery)
    """
    dataset_dir = 'vrid'
    ...
    def __init__(self, root='data', **kwargs):
        ...
        # self.train_dir = osp.join(self.dataset_dir, 'original/train')
        # self.query_dir = osp.join(self.dataset_dir, 'original/query')
        # self.gallery_dir = osp.join(self.dataset_dir, 'original/test')

        self.train_dir = osp.join(self.dataset_dir, 'ce/train')
        self.query_dir = osp.join(self.dataset_dir, 'ce/query')
        self.gallery_dir = osp.join(self.dataset_dir, 'ce/test')

        self.heatmap = osp.join(self.dataset_dir, 'heatmap')
        if not osp.exists(self.heatmap):
    ...
    def _process_dir(self, dir_path, relabel=False):
        ...
        heatmap_save_path = osp.join(self.heatmap, dir_path.split('/')[-1])
        if not osp.exists(heatmap_save_path):
        ...
        for img_path in img_paths:
            ...
            fig = plt.figure()
            plt.imshow(image, cmap = cm.jet, alpha=0.5)   
            plt.colorbar()
            plt.savefig('{}/{}'.format(heatmap_save_path, img_path.split('/')[-1]))
...
class PKU_VD(object):
    ...
    PKU_VD
    Reference:
	Yan et al. Exploiting Multi-Grain Ranking Constraints for Precisely Searching Visually-Similar Vehicles, ICCV 2017
    ...
    def __init__(self, root='data', **kwargs):
        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.train_dir = osp.join(self.dataset_dir, 'VD2/train')
        self.query_dir = osp.join(self.dataset_dir, 'VD2/query')
        self.gallery_dir = osp.join(self.dataset_dir, 'VD2/test')
    ...
    def _process_dir(self, dir_path, relabel=False):
        ...
        for img_path in img_paths:
            ...
            pid, _ = map(int, pattern.search(img_path).groups())
            if pid == -1: continue  # junk images are just ignored
            assert 0 <= pid <= 10**6  # pid == 0 means background
            # assert 0 <= camid <= 6
            # camid -= 1 # index starts from 0
            if relabel: pid = pid2label[pid]
            dataset.append((img_path, pid, _))
...
class CampCars(object):
    ...
    def _process_dir(self, dir_path, relabel=False):
        ...
        #     if img_split == 'unknown':
        #         img_split = '127'
        #         #替换文件列表的每一行
                
        #         dirname, _ = osp.split(file)
        #         new_filename = img_name.split('_')[0]  + '_' + img_split + '_' +  img_name.split('_')[2] + '.jpg'
        #         print('new_filename', new_filename)
        #         file = dirname + '/' + new_filename
        #         print('file', file)
        #         # file = file.split('/')[0] + '/' + file.split('/')[1] + '/' + file.split('/')[2] + '/' + \
        #         #         img_name.split('_')[0]  + '_' + img_split + '_' +  img_name.split('_')[2]
        #         #重命名原来文件
        #         os.rename(img_paths[i], file)
        #         #找到原路径列表中的可替换行的位置
        #         img_paths[i] = file
        #     #遍历所有要替换的行
        #     i +=1
...
class BoxCars116k(object):
    """
    BoxCars116k

    Reference:
    1.BoxCars: Improving Fine-Grained Recognition of Vehicles using 3D Bounding Boxes in Traffic Surveillance [IEEE T-ITS 2018]
    2.BoxCars: 3D Boxes as CNN Input for Improved Fine-Grained Vehicle Recognition [CVPR 2016]

    URL: https://github.com/JakubSochor/BoxCars
    
    """
    dataset_dir = 'boxcars116k'

    def __init__(self, root='data', **kwargs):
        self.dataset_dir = osp.join(root, self.dataset_dir)

        # self.train_dir = osp.join(self.dataset_dir, 'dataset-delete<=2/train')
        # self.query_dir = osp.join(self.dataset_dir, 'dataset-delete<=2/query')
        # self.gallery_dir = osp.join(self.dataset_dir, 'dataset-delete<=2/test')
        self.train_dir = osp.join(self.dataset_dir, 'dataset-delete<=3/train')
        self.query_dir = osp.join(self.dataset_dir, 'dataset-delete<=3/query')
        self.gallery_dir = osp.join(self.dataset_dir, 'dataset-delete<=3/test')
...
class DukeMTMCreID(object):
    ...
    def _process_dir(self, dir_path, relabel=False):
        img_paths = glob.glob(osp.join(dir_path, '*.jpg'))    # 函数功能：匹配所有的符合条件的文件，并将其以list的形式返回
        pattern = re.compile(r'([-\d]+)_c(\d)')
...
def get_names():
    # return __img_factory.keys() + __vid_factory.keys()            #python2
    return list(__img_factory.keys()) + list(__vid_factory.keys())  #python3
