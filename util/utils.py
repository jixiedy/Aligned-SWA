...
def save_checkpoint_test(dir, epoch, **kwargs):
    state = {
        'epoch': epoch,
    }
    state.update(kwargs)
    filepath = os.path.join(dir, 'checkpoint-%d.pth' % epoch)
...
# def save_checkpoint1(state1, is_best1, fpath='checkpoint.pth.tar', **kwargs):
def save_checkpoint1(state1, is_best1, fpath='checkpoint.pth.tar'):
    #当选择更新时：state1.update(kwargs)，后面的参数会覆盖住前面的参数，导致保存的模型体积很小
    # state1 = {}
    # state1.update(kwargs)
    mkdir_if_missing(osp.dirname(fpath))
    torch.save(state1, fpath)
    if is_best1:
        shutil.copy(fpath, osp.join(osp.dirname(fpath), 'best1_model.pth.tar'))

# def save_checkpoint2(state2, is_best2, fpath='checkpoint.pth.tar', **kwargs):
def save_checkpoint2(state2, is_best2, fpath='checkpoint.pth.tar'):
    # state2 = {}
    # state2.update(kwargs)
    mkdir_if_missing(osp.dirname(fpath))
    torch.save(state2, fpath)
    if is_best2:
        shutil.copy(fpath, osp.join(osp.dirname(fpath), 'best2_model.pth.tar'))        
...
def bn_update(loader, model):
    ...
    n = 0
    for input, pids, _ in loader:
        input = input.cuda(device = None, non_blocking = False)  # https://stackoverflow.com/questions/53201534/calling-cuda-with-async-results-in-syntaxerror
        # input = input.cuda(async=True)  # async和await从 python3.7 开始已经加入保留关键字中. 所以async和await不能作为函数的参数名.
